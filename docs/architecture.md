...
Compass is a native macOS host around a local software factory loop and a repo health improver.
...
- **OpenAI-compatible cloud** (`OpenAICompatibleModelRuntime`) — primary for Plan / Develop / Critic / Requirements Audit / Health.
...
Coverage snapshots are persisted via `CoverageSnapshotStore` after verify; mutation results via `MutationSnapshotStore` (`MutationReportParser` extracts kill-rate and surviving mutants from `cargo mutants` output). Health snapshots via `HealthSnapshotStore` after every successful ship (and pure health runs). Coverage, mutation, and health snapshots all feed the next Plan prompt. Plan handoff validation uses `GeneratedVerifyValidator` for coverage-ready verify commands.
