...
final class CompassProjectMutationTestingTests: XCTestCase {
  ...
  func testSuccessfulExecutionUsesInjectedRunnerAndPersistsMutationRecord() async throws {
    ...
    XCTAssertEqual(
      session.latestExecutionEnvironmentSnapshot?.effectiveRouteIdentifier, "native-macos")
    ...
    XCTAssertTrue(
      project.runtimeDiagnosticsMenu.copyDiagnosticsAction.copyText.contains(
        "mutation-recovery-state: succeeded"))
  }
  ...
  func testFailingExecutionRecordsBoundedRedactedMetadataWithoutLeakingRuntimeValues() async throws
  {
    ...
    XCTAssertLessThanOrEqual(
      execution.outputTail.count, SessionMutationTestingExecution.outputTailLimit)
    XCTAssertLessThanOrEqual(
      execution.seedCommandLabel.count, AgentMutationTestingPlan.commandMaxCharacters)

    let persistedText = try read(workspace.sessionsRecordURL)
    let exposedText = [persistedText, execution.outputTail, execution.seedCommandLabel].joined(
      separator: "\n")
