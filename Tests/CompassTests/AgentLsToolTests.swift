...
final class AgentLsToolTests: XCTestCase {
    ...
    func testListsFilesAndDirectoriesWithTrailingSlashOnDirs() async throws {
        try "a".write(to: temporaryDirectory.appendingPathComponent("alpha.txt"), atomically: true, encoding: .utf8)
        try "b".write(to: temporaryDirectory.appendingPathComponent("beta.txt"), atomically: true, encoding: .utf8)
    ...
    func testListsHiddenEntries() async throws {
        try "x".write(to: temporaryDirectory.appendingPathComponent(".compass"), atomically: true, encoding: .utf8)
        try "y".write(to: temporaryDirectory.appendingPathComponent("visible.txt"), atomically: true, encoding: .utf8)
    ...
    func testRejectsRegularFile() async throws {
        try "x".write(to: temporaryDirectory.appendingPathComponent("file.txt"), atomically: true, encoding: .utf8)
