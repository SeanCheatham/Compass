...
final class AgentMutationTestingPlanTests: XCTestCase {
  ...
  func testMissingAndUnsupportedStatesAreBounded() {
    ...
    for plan in [noImmediate, missingVerify, unsupported] {
      XCTAssertLessThanOrEqual(
        plan.identifier.count, AgentMutationTestingPlan.identifierMaxCharacters)
  ...
  func testIdentifierCopyAndSeedCommandAreStableBoundedAndSanitized() throws {
    ...
    let state = makeState(
      verify:
        "swift test --package-path \(repoURL.path) --filter AgentMutationTestingPlanTests \(secretEnvValue)"
    )
    ...
    XCTAssertLessThanOrEqual(
      first.identifier.count, AgentMutationTestingPlan.identifierMaxCharacters)
    XCTAssertLessThanOrEqual(
      first.seedCommandLabel.count, AgentMutationTestingPlan.commandMaxCharacters)
