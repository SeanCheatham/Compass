...
final class CompassWorkspaceStoragePreflightTests: XCTestCase {
    ...
    func testCurrentAndLegacyApplicationSupportConflictsAreInspectOnly() throws {
        ...
        XCTAssertEqual(try entries(in: seedPreflight.currentApplicationSupportCandidateURL), currentEntriesBefore)
        XCTAssertEqual(try entries(in: seedPreflight.legacyApplicationSupportCandidateURL), legacyEntriesBefore)
        XCTAssertEqual(
            try String(contentsOf: seedPreflight.currentApplicationSupportCandidateURL.appending(path: "state.json"), encoding: .utf8),
            "current\n"
        )
        XCTAssertEqual(
            try String(contentsOf: seedPreflight.legacyApplicationSupportCandidateURL.appending(path: "state.json"), encoding: .utf8),
            "legacy\n"
        )
    }
    ...
    func testPreflightDoesNotCreateRepoOrApplicationSupportFiles() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: preflight.currentApplicationSupportCandidateURL.path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: preflight.legacyApplicationSupportCandidateURL.path))
    }
    ...
    private func makeTemporaryDirectory(prefix: String = "CompassWorkspaceStoragePreflightTests") throws -> URL {
