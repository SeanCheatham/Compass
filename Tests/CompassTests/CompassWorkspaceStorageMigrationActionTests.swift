...
final class CompassWorkspaceStorageMigrationActionTests: XCTestCase {
    ...
    func testInjectedMigrationFailureShowsBoundedFailureFeedback() async throws {
        ...
        XCTAssertLessThanOrEqual(project.storageMigrationState.label.count, CompassProjectStorageMigrationState.labelLimit)
        XCTAssertLessThanOrEqual(project.storageMigrationState.detail.count, CompassProjectStorageMigrationState.detailLimit)
        XCTAssertLessThanOrEqual(project.storageMigrationState.helpText.count, CompassProjectStorageMigrationState.helpLimit)
    ...
    func testUnavailableAndRunningPlansBlockWithoutCallingMigrator() async throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: missingProject.storageMigrationPlan().destinationURL.path))
    ...
    func testConfirmationAndStateTextStayBounded() async throws {
        ...
        let roots = try makeApplicationSupportRoots(
            prefix: "CompassWorkspaceStorageMigrationActionSupport" + String(repeating: "LongSegment", count: 12)
        )
        try CompassWorkspace(repoURL: repoURL).initialize()
        let project = CompassProject(
            repoURL: repoURL,
            storageApplicationSupportRoots: roots,
            storageMigrationAction: { _ in
                throw InjectedMigrationActionError.failed(String(repeating: "long-user-facing-error-", count: 70))
            }
        )
        ...
        XCTAssertLessThanOrEqual(confirmation.title.count, CompassWorkspaceStorageMigrationConfirmation.titleLimit)
        XCTAssertLessThanOrEqual(confirmation.message.count, CompassWorkspaceStorageMigrationConfirmation.messageLimit)
        XCTAssertLessThanOrEqual(confirmation.confirmLabel.count, CompassWorkspaceStorageMigrationConfirmation.actionLabelLimit)
        XCTAssertLessThanOrEqual(confirmation.cancelLabel.count, CompassWorkspaceStorageMigrationConfirmation.actionLabelLimit)
        XCTAssertLessThanOrEqual(project.storageMigrationState.label.count, CompassProjectStorageMigrationState.labelLimit)
        XCTAssertLessThanOrEqual(project.storageMigrationState.detail.count, CompassProjectStorageMigrationState.detailLimit)
        XCTAssertLessThanOrEqual(project.storageMigrationState.helpText.count, CompassProjectStorageMigrationState.helpLimit)
    ...
    func testActionPreservesRepoLocalStorageAndDoesNotSwitchActiveStorage() async throws {
        ...
        XCTAssertEqual(try read(workspace.sessionsURL.appending(path: "1-transcript.txt")), "source artifact\n")
    ...
    private func makeTemporaryDirectory(prefix: String = "CompassWorkspaceStorageMigrationActionTests") throws -> URL {
