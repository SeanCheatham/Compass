...
struct ProductTournamentLoopTests {
  ...
  @MainActor @Test func tournamentAutomationRetargetsStalledActedProofGroup() async throws {
    ...
    let revisionAudit = TournamentAutomationCycleAudit(
      id: "tournament-cycle-stalled-acted-revision",
      startedAt: 140,
      endedAt: 150,
      executedStepIDs: [nextStep.id],
      experimentIDs: [experiment.id],
      messages: ["Applied acted proof-group revision."],
      maxSteps: 1,
      revisionBriefSummaries: [revisionBrief.auditSummary],
      stopReason: .reachedStepLimit,
      stopStepID: nextStep.id,
      stopStepTitle: nextStep.title,
    ...
    try #require(revisionBrief.auditSummary.contains("validation Direct proof stall"))
    ...
    try #require(digest.contains("validation Direct proof stall"))
  ...
  @MainActor @Test func tournamentAutomationRetargetsRepeatedStillActedProofGroup() async throws {
    ...
    let group = try #require(
      scoreboardItem.readinessGroup(containingRowSelectionID: row.selectionID))
    let actedSummary = group.actionAuditSummary(anchorRow: row)

    for (offset, auditID) in [
      (0.0, "tournament-cycle-still-acted-group-a"),
      (10.0, "tournament-cycle-still-acted-group-b"),
    ] {
      config = config.recordingTournamentAutomationCycleAudit(
        TournamentAutomationCycleAudit(
          id: auditID,
          startedAt: 120 + offset,
          endedAt: 125 + offset,
          executedStepIDs: [
            "\(experiment.id):\(ProductTournamentNextActionKind.runCohort.rawValue):\(targetScenarioID)"
          ],
          experimentIDs: [experiment.id],
          messages: ["persona-model proof group ran, but the acted group stayed present."],
          maxSteps: 3,
          evidenceRunStepCount: 1,
          evidenceRunIDs: ["buyer-persona-model-still-\(Int(offset))"],
          completedEvidenceRunCount: 1,
          failedEvidenceRunCount: 0,
          skippedScenarioCount: 0,
          actedProofPressureGroupSummaries: [actedSummary],
          stopReason: .noExecutableStep,
          stopDetail: "Stopped because no executable tournament automation step remains.",
    ...
    let repeatedGroup = try #require(
      TournamentAutomationCycleLearningAdvisor.repeatedActedPressureGroupAudit(
        for: proofRunAction,
        experiment: experiment,
        config: config,
        evidenceIndex: evidenceIndex
    ...
    let workbenchRevisionBrief = try #require(
      ...
      .first { $0.experimentID == experiment.id })
    let revisionAudit = TournamentAutomationCycleAudit(
      id: "tournament-cycle-repeated-acted-revision",
      startedAt: 150,
      endedAt: 160,
      executedStepIDs: [nextStep.id],
      experimentIDs: [experiment.id],
      messages: ["Applied repeated acted proof-group revision."],
      maxSteps: 1,
      revisionBriefSummaries: [revisionBrief.auditSummary],
      stopReason: .reachedStepLimit,
      stopStepID: nextStep.id,
    ...
    try #require(repeatedGroup.outcome.summary.contains("still Proof runs"))
    try #require(retarget.kind == .refineContender)
    try #require(retarget.title == "Retarget repeated proof group")
    try #require(retarget.detail.contains("tournament-cycle-still-acted-group-b"))
    try #require(retarget.detail.contains("still Proof runs"))
    try #require(retarget.targetScenarioID == targetScenarioID)
    ...
    try #require(revisionBrief.auditSummary.contains("source acted_proof_group"))
    try #require(
      revisionBrief.auditSummary.contains(
        "validation Repeated still-present proof pressure"))
    try #require(
    ...
    try #require(digest.contains("Tournament automation revision briefs"))
    try #require(digest.contains("source acted_proof_group"))
    try #require(digest.contains("validation Repeated still-present proof pressure"))
