...
struct RustProjectScaffoldTests {
  @Test func scaffoldUsesBlessedCargoWorkspaceShape() throws {
    ...
    try #require(paths.contains("schemas/productization-experience-input.schema.json"))
    try #require(paths.contains("schemas/productization-experience-trace.schema.json"))
  ...
  @Test func scaffoldDocumentsStandardRustCommandsAndDesktopStack() throws {
    ...
    try #require(readme.contains("Deterministic Productization Experience Contract"))
    try #require(readme.contains("run_productization_experience(ProductizationExperienceInput) ->"))
    try #require(readme.contains("app-cli productization-experience --input"))
    try #require(readme.contains("Productization evidence is product pressure"))
    try #require(readme.contains("Manual productization simulation checklist"))
    try #require(readme.contains("allowedNextActions"))
    try #require(readme.contains("avoid optimizing for praise"))
    try #require(readme.contains(RustVerifyCommands.cargo(RustVerifyCommands.productizationSmoke)))
  ...
  @Test func appCoreTemplateExposesDeterministicSimulationFixture() throws {
    ...
    try #require(core.contains("pub struct ProductizationExperienceInput"))
  ...
  @Test func xtaskImplementsLevelTwoVisualVerificationSteps() throws {
    ...
    try #require(xtask.contains(#""productization-smoke" | "pmf-smoke" => productization_smoke()"#))
    try #require(xtask.contains("fn productization_smoke() -> Result<()>"))
    ...
    try #require(xtask.contains("productization_smoke()?"))
  ...
  @Test func generatedRustScaffoldCargoSmokeWhenRequested() async throws {
    ...
    try await requireCommand(["run", "-p", "xtask", "--", "productization-smoke"], in: root)
