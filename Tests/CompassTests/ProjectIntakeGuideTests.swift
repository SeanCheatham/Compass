...
struct ProjectIntakeGuideTests {
  ...
  func firstProjectGuideExplainsGitIntake() throws {
    ...
    try #require(guide.steps[2].title == "Seed the proof loop")
    try #require(guide.steps[2].detail.contains("proof ideas"))
  ...
  func clipboardPayloadPackagesPlainLanguageHandoff() throws {
    ...
    try #require(payload.text.contains("- Seed the proof loop:"))
    try #require(payload.text.contains("Pain context:"))
    try #require(payload.text.contains("proof ideas or validation questions"))
