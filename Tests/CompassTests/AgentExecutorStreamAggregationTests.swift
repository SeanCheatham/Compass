...
struct AgentExecutorStreamAggregationTests {
  ...
  func keepsToolCallsOrderedByIndex() async throws {
    ...
    let executor = AgentExecutor()
    let turn = try await executor.aggregate(stream: chunks)
    try #require(turn.toolCalls.map { $0.id } == ["call_a", "call_b"])
  }
  ...
  func dropsToolCallsMissingIdOrName() async throws {
    ...
    let executor = AgentExecutor()
    let turn = try await executor.aggregate(stream: chunks)
    try #require(turn.toolCalls.map { $0.id } == ["call_b"])
  }

  // MARK: - Cancellation

  @Test
  func cancelledExecutorThrowsBeforeConsumingStream() async throws {
    let executor = AgentExecutor()
    executor.cancel()
