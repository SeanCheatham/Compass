...
final class CompassWorkspaceStorageBoundaryTests: XCTestCase {
    ...
    func testBoundaryDisplayTextAndIdentifiersStayBounded() throws {
        ...
        let roots = KnownProjectStore.ApplicationSupportRoots(
            current: URL(fileURLWithPath: "/tmp/" + String(repeating: "Current Support Root/", count: 12)),
        ...
        XCTAssertLessThanOrEqual(boundary.recommendation.count, CompassWorkspaceStorageBoundary.recommendationLimit)
    ...
    func testStableIdentifiersFlowThroughAssessmentPreflightAndBoundary() throws {
        ...
        XCTAssertEqual(boundary.currentApplicationSupportCandidateURL, assessment.currentApplicationSupportCandidateURL)
        XCTAssertEqual(boundary.currentApplicationSupportCandidateURL, preflight.currentApplicationSupportCandidateURL)
        XCTAssertEqual(boundary.legacyApplicationSupportCandidateURL, assessment.legacyApplicationSupportCandidateURL)
        XCTAssertEqual(boundary.legacyApplicationSupportCandidateURL, preflight.legacyApplicationSupportCandidateURL)
        XCTAssertEqual(boundary.currentApplicationSupportCandidateURL.lastPathComponent, boundary.projectStorageIdentifier)
        XCTAssertEqual(boundary.legacyApplicationSupportCandidateURL.lastPathComponent, boundary.projectStorageIdentifier)
    }
    ...
    func testBoundaryConstructionCreatesNoRepoOrApplicationSupportFiles() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: boundary.currentApplicationSupportCandidateURL.path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: boundary.legacyApplicationSupportCandidateURL.path))
    }
    ...
    private func makeTemporaryDirectory(prefix: String = "CompassWorkspaceStorageBoundaryTests") throws -> URL {
