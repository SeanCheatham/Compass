...
struct ProductTournamentRoundEvidenceTransitionTests {
  @Test func strongFeasibilityEvidenceAdvancesContenderToProductImplementationRound() throws {
    ...
    try #require(proposal.nextValidationTarget.contains("Round 3 product implementation"))
    ...
    try #require(proofOverviewItem.helpSummary.contains("Next validation"))
  ...
  @Test func singleFeasibilityRunOnlyRecommendsGatheringMoreEvidence() throws {
    ...
    try #require(proposal.proofGaps.contains { $0.contains("needs 1 more persona") })
    try #require(proposal.proofGaps.contains { $0.contains("needs 1 completed-use trace") })
    try #require(proposal.nextValidationTarget.contains("Run scoped Round 2 evidence"))
  ...
  @Test func strongFeasibilityScoresWithoutUseProofOnlyGatherEvidence() throws {
    ...
    try #require(proposal.proofGaps.contains { $0.contains("needs 2 completed-use trace") })
    try #require(proposal.nextValidationTarget.contains("completed-use trace"))
  ...
  @Test func roundTwoProofOverviewShowsActiveCoreTechnologyProofBeforeEvidence() throws {
    ...
    try #require(item.displayDetail.contains("Next validation"))
    ...
    try #require(item.contextLine.contains("proof_gaps no scoped Round 2 evidence yet"))
    ...
    try #require(item.nextValidationTarget.contains("Run scoped Round 2 evidence"))
