...
final class AgentVsockClientTests: XCTestCase {
    ...
    func testListDirectoryParsesEntries() async throws {
        let transport = StubTransport(responses: [makeFrame(.listDirectory(.init(entries: [
    ...
    func testGlobReturnsMatchesWithDates() async throws {
        let transport = StubTransport(responses: [makeFrame(.glob(.init(matches: [
    ...
    func testRunPassesCommandAndCwdInRequest() async throws {
        let transport = StubTransport(responses: [makeFrame(.bash(.init(
            exitCode: 0,
            stdout: "hi\n",
            stderr: ""
        )))])
