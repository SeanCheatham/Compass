...
struct ProductTournamentEvidenceStoreTests {
  ...
  @Test func storeWritesPlanEvaluationArtifactsAndIndex() throws {
    ...
    let record = ProductTournamentPlanEvaluationRecord(
      ...
      contenderPlanID: "plan-reporting-desk",
  ...
  @Test func storePreservesDecisionIntentAcrossRecordIndexMarkdownAndDigest() throws {
    ...
    let record = makeEvidenceRecord(
      ...
      painID: config.contenderPlans[0].painID,
  ...
  @Test func planAndReflectPromptsIncludeProductTournamentContextWithoutTranscripts() throws {
    ...
    try #require(plan.contains("## PMF Proof Context"))
    try #require(plan.contains("PMF Context Packs"))
    try #require(plan.contains("proof_core"))
    try #require(plan.contains("evidence_slice"))
    try #require(plan.contains("tool_budget"))
    try #require(plan.contains("Product Tournament Context is minimized"))
    try #require(plan.contains(config.tournamentExperiments[0].branchName))
    try #require(plan.contains("prompt-run-two"))
    try #require(plan.contains("persona_rationale"))
    try #require(plan.contains("Needed import proof before trusting a switch"))
    try #require(plan.contains("Still needs import proof before switching"))
    try #require(plan.contains("csv_import"))
    try #require(!plan.contains("Next tournament automation actions"))
    try #require(!plan.contains("kind run_cohort"))
    try #require(!plan.contains("cohort \(config.scenarioCohorts[0].id)"))
    try #require(!plan.contains("Repeated objections"))
    try #require(!plan.contains("the spreadsheet is already trusted (2x)"))
    try #require(!plan.contains("csv_import (2x)"))
    try #require(!plan.contains("transcript.jsonl"))
    try #require(!plan.contains(#""turn":0"#))

    try #require(reflect.contains("## Product Tournament Context"))
    try #require(reflect.contains(config.tournamentExperiments[0].branchName))
    try #require(reflect.contains("Next tournament automation actions"))
    try #require(reflect.contains("kind run_cohort"))
    try #require(reflect.contains("cohort \(config.scenarioCohorts[0].id)"))
    try #require(reflect.contains("mode model_free"))
    try #require(reflect.contains("prompt-run-two"))
    try #require(reflect.contains("persona_rationale"))
    try #require(reflect.contains("Needed import proof before trusting a switch"))
    try #require(reflect.contains("Repeated objections"))
    try #require(reflect.contains("the spreadsheet is already trusted (2x)"))
    try #require(reflect.contains("csv_import (2x)"))
    try #require(reflect.contains("Beat the spreadsheet"))
    try #require(!reflect.contains("transcript.jsonl"))
    try #require(!reflect.contains(#""turn":0"#))
  }
  ...
  @Test func productTournamentSmokeConnectsWorkspaceWorkbenchPromptAndRollout() async throws {
    ...
    try #require(prompt.contains("evidence_slice"))
...
private func makeEvidenceRecord(
  ...
  contenderPlanID: String = "plan-one",
