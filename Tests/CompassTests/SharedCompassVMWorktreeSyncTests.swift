...
struct SharedCompassVMWorktreeSyncTests {
  ...
  @Test func allowedGuestPathPrefixesAreOnlyTheCatalogRoot() {
    #require(
  ...
  @Test func deletionsLeaveUserAddedFilesAlone() {
    let deletions = SharedCompassVMWorktreeSync.computeDeletions(
  ...
  @Test func deletionsRequirePresenceOnHost() {
    let deletions = SharedCompassVMWorktreeSync.computeDeletions(
      host: ["a.swift"],
      guest: [],
      scope: ["a.swift", "b.swift"]
    )
    #require(deletions == ["a.swift"])
  }
  ...
  @Test func nilScopeFallsBackToHostMinusGuest() {
    let deletions = SharedCompassVMWorktreeSync.computeDeletions(
      host: ["a.swift", "b.swift", "user-new.txt"],
      guest: ["a.swift"],
      scope: nil
    )
    #require(deletions == ["b.swift", "user-new.txt"])
  }

  @Test func excludesSyncPathSkipsBuildArtifactTrees() {
    #require(SharedCompassVMWorktreeSync.excludesSyncPath(".build"))
    #require(SharedCompassVMWorktreeSync.excludesSyncPath(".build/debug"))
    #require(
      SharedCompassVMWorktreeSync.excludesSyncPath(".build/arm64-apple-macosx/debug/App.swift"))
    #require(!SharedCompassVMWorktreeSync.excludesSyncPath("Sources/App.swift"))
    #require(!SharedCompassVMWorktreeSync.excludesSyncPath("build.gradle"))
  }
}