...
enum AgentToolError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case let .invalidArguments(detail): return "Invalid arguments: \(detail)"
        case let .pathEscapesWorkingDirectory(path): return "Path escapes the working directory: \(path)"
        case let .fileNotFound(path): return "File not found: \(path)"
        case let .notRegularFile(path): return "Not a regular file: \(path)"
        case let .notDirectory(path): return "Not a directory: \(path)"
        case let .binaryFile(path): return "Cannot read binary file: \(path)"
