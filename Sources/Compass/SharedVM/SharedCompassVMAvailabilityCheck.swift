...
enum SharedCompassVMAvailabilityCheck {
    ...
    /// installer) into a user-readable availability reason. The agent
    /// runtime uses this to translate VZ errors into the
    ...
    static func describe(error: Error) -> String {
        ...
        if nsError.domain == VZErrorDomain {
            ...
            case .virtualMachineLimitExceeded:
                return "Apple's per-host VM limit was reached. Quit other VM apps (Parallels, UTM, Xcode device VMs) and try again."
            case .networkError:
            ...
            case .internalError:
                return "Virtualization framework reported an internal error: \(nsError.localizedDescription)"
