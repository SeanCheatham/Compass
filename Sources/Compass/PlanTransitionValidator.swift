    ...
    static func validate(from current: PlanState, to next: PlanState) throws {
        if next.completed.count < current.completed.count {
            throw PlanTransitionValidationError(
                message: "Plan tried to shrink completed history from \(current.completed.count) entries to \(next.completed.count). Refusing to overwrite state.json."
            )
        }
        ...
        if !current.midTerm.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty &&
            ...
            throw PlanTransitionValidationError(
                message: "Plan tried to clear a non-empty midTerm queue without recording a completion. Refusing to overwrite state.json."
            )
        }
        ...
        if rejectedVerifyCommands.contains(verify) {
            throw PlanTransitionValidationError(
                message: "Plan returned placeholder verify command `\(immediate.verify)`. Refusing to overwrite state.json."
            )
        }
    }
}
