...
enum MutationTestingPresentationSanitizer {
    ...
    private static func redactSensitiveText(_ text: String) -> String {
        ...
        let replacements: [(pattern: String, template: String)] = [
            (#"(?:(?:file://)?/(?:Users|private|var|tmp|opt|usr|bin|sbin|Library|Applications|Volumes)/[^\s,;)`"']+)"#, "[path]"),
            (#"(^|[\s"'=:/])(?:\./)?\.devcontainer/[^\s"']+"#, "$1[devcontainer-path]"),
            (#"(^|[\s"'=:/])\.\./[^\s"']+"#, "$1[path]"),
            (#"ghcr\.io/devcontainers/features/[^\s,;)`"']+"#, "[feature]"),
            (#"(?i)(^|[\s"'=:/])(?:\.\./|\./)?(?:docker-)?compose[^\s"']*\.ya?ml"#, "$1[compose-path]"),
            (#"(?i)\b(containerEnv|container_env|buildArg|build-arg|build_arg|feature-option|featureOption|token|secret|password|passwd|api[_-]?key)\b\s*[:=]\s*[^\s,;)`"']+"#, "$1=[redacted]"),
            (#"(?i)\b[A-Za-z0-9_.-]*(?:secret|token|password|passwd|apikey|api-key)[A-Za-z0-9_.-]*\b"#, "[redacted]")
        ]
