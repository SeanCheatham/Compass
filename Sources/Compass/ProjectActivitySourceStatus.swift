...
struct ProjectActivitySourceStatus: Equatable {
    ...
    init(snapshot: RepositoryActivitySourceSnapshot) {
        ...
        helpText = presentation.kind == .hidden
            ? ""
            : Self.boundedText(
    ...
    private static func unavailablePresentation(
        ...
        let storageName = snapshot.activeStorage == .applicationSupport
            ? "Application Support"
            : "repo-local .compass/"
        let kind: Kind = snapshot.activeStorage == .applicationSupport
        ...
        case .notScanned:
            detail = "\(storageName) sessions.json has not been scanned yet; cinematic activity is pending."
        case .storageRootMissing:
            detail = "\(storageName) activity root is missing; cinematic activity uses an empty source until storage returns."
        case .sessionsRecordMissing:
            detail = "\(storageName) sessions.json is missing; cinematic activity uses an empty source until the record returns."
