...
struct NativeFeedbackDeliverySnapshot: Equatable {
  ...
  init(
    ...
    self.authorizationRequestStateIdentifier = Self.boundedIdentifier(
      authorizationRequestStateIdentifier)
...
struct NativeFeedbackModeMenu: Equatable {
  ...
  private static func permissionHint(for mode: NativeFeedbackMode, projectName: String) -> String {
    ...
    case .notifications:
      copy =
        "Compass asks notification permission for \(projectName) only when enabled or first delivered."
    case .speechAndNotifications:
      copy =
  ...
  private static func deliveryStatusText(for snapshot: NativeFeedbackDeliverySnapshot?) -> String {
    guard let snapshot else {
      return bounded(
        "Delivery diagnostics unavailable", limit: NativeFeedbackDeliverySnapshot.menuStatusLimit)
    }
...
final class NativeFeedbackService: NSObject, UNUserNotificationCenterDelegate {
  ...
  func emit(
    ...
    let content =
      providedContent ?? NativeFeedbackContent(milestone: milestone, projectName: projectName)
    if mode.sendsNotifications {
      lastAttemptResultIdentifier =
        mode.speaks ? "queued-notification-speech" : "queued-notification"
      Task { @MainActor in
  ...
  func deliverySnapshot(mode: NativeFeedbackMode) -> NativeFeedbackDeliverySnapshot {
    NativeFeedbackDeliverySnapshot(
      mode: mode,
      notificationSupportIdentifier: notificationCenter == nil
        ? "unavailable-app-bundle" : "available",
  ...
  private func deliverNotification(_ content: NativeFeedbackContent) async {
    ...
    guard notificationsAllowed else {
      lastAttemptResultIdentifier =
        "notification-suppressed-\(notificationAuthorizationStatusIdentifier)"
      return
    }
