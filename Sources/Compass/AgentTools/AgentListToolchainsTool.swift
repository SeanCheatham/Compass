...
struct AgentListToolchainsTool: AgentTool {
  ...
  static func format(_ statuses: [ToolchainStatus]) -> String {
    ...
    lines.append("")
    lines.append(
      "Install missing on-demand toolchains with install_toolchain. Unknown means Compass could not probe the guest right now. Default-provisioned toolchains (command_line_tools, homebrew, ripgrep, rust) are installed during VM setup; the Rust stack includes compass-engine for structured Cargo tooling."
    )
