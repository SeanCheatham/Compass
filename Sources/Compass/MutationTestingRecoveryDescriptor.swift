...
struct MutationTestingRecoveryDescriptor: Equatable, Identifiable {
  ...
  private init(
    ...
    let statusIdentifier =
      execution.map(Self.statusIdentifier) ?? readiness?.statusIdentifier ?? "missing"
    let statusLabel =
      execution.map {
        MutationTestingPresentationSanitizer.statusLabel(Self.statusIdentifier(for: $0))
      }
      ?? readiness?.statusLabel
      ?? "Missing"
    let routeIdentifier =
      execution.map { MutationTestingPresentationSanitizer.routeIdentifier($0.routeIdentifier) }
      ?? readiness?.routeIdentifier
      ?? "unknown"
    let routeLabel =
      execution.map { _ in MutationTestingPresentationSanitizer.routeLabel(routeIdentifier) }
      ?? readiness?.routeLabel
      ?? "Unknown route"
    let languageIdentifier =
      execution.map {
        MutationTestingPresentationSanitizer.languageIdentifier($0.languageIdentifier)
      }
      ?? readiness?.languageIdentifier
      ?? "unknown"
    let languageLabel =
      execution.map { _ in MutationTestingPresentationSanitizer.languageLabel(languageIdentifier) }
      ?? readiness?.languageLabel
      ?? "Unknown"
    let seedCommandLabel =
      execution.map {
      ...
      ?? "none"
    let exitCodeText =
      execution.map { MutationTestingPresentationSanitizer.exitCodeLabel($0.exitCode) }
      ?? "exit none"
    let durationText =
      execution.map {
    ...
    self.seedCommandLabel = Self.bounded(
      seedCommandLabel, limit: SessionMutationTestingExecution.commandLimit)
  ...
  private static func copy(
    ...
    title: String, help: String, reviewActionLabel: String, copyActionLabel: String,
    systemImage: String
  ) {
    switch state {
    case .active:
  ...
  private static func detailText(
    ...
    case .active:
      ...
      return
        "Latest mutation run failed via \(routeLabel) for \(languageLabel); seed \(seedCommandLabel) ended \(exitCodeText)."
    case .succeeded:
      return
        "Latest mutation run succeeded via \(routeLabel) for \(languageLabel); no recovery cue is active."
    case .oldSession:
      return
        "Older failed mutation run via \(routeLabel) remains in history only because a newer mutation run exists."
    case .readinessOnly:
