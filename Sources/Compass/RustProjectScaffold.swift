...
struct RustProjectScaffold: Equatable, Sendable {
  ...
  private static func readme(projectName: String) -> String {
    ...
    the pain, contender, experiment branch, target commit, current workflow,
    alternatives, and tournament decision intent being evaluated. The generated app owns the allowed action list for each state,
  ...
  private static func productTournamentExperienceInputSchema() -> String {
    ...
    {
      "$schema": "https://json-schema.org/draft/2020-12/schema",
      "title": "ProductTournamentExperienceInput",
      "type": "object",
      "required": [
        "schemaVersion",
        "pain",
      ...
      "properties": {
        ...
        "experiment": {
          "type": "object",
          "required": ["id", "branchName", "targetCommitSha", "successSignal"],
          "properties": {
            "id": { "type": "string" },
  ...
  private static func productTournamentExperienceTraceSchema() -> String {
    ...
    {
      "$schema": "https://json-schema.org/draft/2020-12/schema",
      "title": "ProductTournamentExperienceTrace",
      "type": "object",
      "required": [
        "schemaVersion",
        "painID",
  ...
  private static func appCoreLib() -> String {
    ...
    pub struct ProductTournamentExperienceInput {
        ...
        pub contender: ProductTournamentContender,
    ...
    #[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
    pub struct ProductTournamentExperienceAction {
        pub id: String,
    ...
    #[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
    #[serde(rename_all = "camelCase")]
    pub struct ProductTournamentExperienceAllowedAction {
        pub id: String,
        pub label: String,
        pub description: String,
    ...
    #[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
    #[serde(rename_all = "camelCase")]
    pub struct ProductTournamentExperienceState {
        pub id: String,
        pub headline: String,
        pub body: String,
        pub semantic_nodes: Vec<ProductTournamentExperienceNode>,
        pub observations: Vec<String>,
        pub terminal: bool,
    }

    #[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
    pub struct ProductTournamentExperienceNode {
        pub id: String,
        pub role: String,
        pub text: String,
    }
    ...
    #[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
    #[serde(rename_all = "camelCase")]
    pub struct ProductTournamentExperienceTurn {
        pub index: u32,
        pub action: ProductTournamentExperienceAction,
        pub state: ProductTournamentExperienceState,
        pub allowed_next_actions: Vec<ProductTournamentExperienceAllowedAction>,
        pub event_log: Vec<String>,
    }
    ...
    #[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
    #[serde(rename_all = "camelCase")]
    pub struct ProductTournamentExperienceTrace {
        pub schema_version: u8,
        #[serde(rename = "painID")]
        pub pain_id: String,
        #[serde(rename = "contenderID")]
        ...
        pub turns: Vec<ProductTournamentExperienceTurn>,
        pub allowed_next_actions: Vec<ProductTournamentExperienceAllowedAction>,
        pub terminal_status: ProductTournamentExperienceTerminalStatus,
        pub event_log: Vec<String>,
    ...
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
    #[serde(rename_all = "snake_case")]
    pub enum ProductTournamentExperienceTerminalStatus {
        InProgress,
        Completed,
        Abandoned,
        InvalidAction,
    ...
    #[derive(Debug, Clone)]
    struct ProductTournamentExperienceRuntime {
        state: ProductTournamentExperienceState,
        terminal_status: ProductTournamentExperienceTerminalStatus,
        ...
        fn default() -> Self {
            Self {
                ...
                experiment: ProductTournamentExperiment {
                    ...
                    target_commit_sha: "demo-target-commit".to_owned(),
                    success_signal:
    ...
    pub fn run_product_tournament_experience(
        ...
        let mut runtime = ProductTournamentExperienceRuntime {
            ...
            event_log: vec![
                ...
                format!("contender:{}", input.contender.id),
                format!("experiment:{}", input.experiment.id),
                format!("branch:{}", input.experiment.branch_name),
                format!("target_commit:{}", input.experiment.target_commit_sha),
        ...
        for (index, action) in input.actions.iter().cloned().enumerate() {
            ...
            let turn_events = if allowed_before.iter().any(|allowed| allowed.id == action.id) {
                ...
                runtime.terminal_status = ProductTournamentExperienceTerminalStatus::InvalidAction;
            ...
            turns.push(ProductTournamentExperienceTurn {
                index: index as u32,
                action,
            ...
            if runtime.terminal_status != ProductTournamentExperienceTerminalStatus::InProgress {
                break;
            }
        }
        ...
        ProductTournamentExperienceTrace {
            ...
            contender_id: input.contender.id,
            experiment_id: input.experiment.id,
            branch_name: input.experiment.branch_name,
            target_commit_sha: input.experiment.target_commit_sha,
            initial_state,
            turns,
    ...
    fn initial_product_tournament_experience_state(
        ...
        product_tournament_experience_state(
            ...
            format!(
                "{} is trying to: {}. Current workflow: {}. Contender: {}. Decision intent: {}.",
                input.scenario.persona_summary,
                input.scenario.task,
                input.current_workflow.summary,
                input.contender.promise,
                decision_intent
            ),
            vec![
                format!("pain visible: {}", input.pain.summary),
                format!("implementation target: {} @ {}", input.experiment.branch_name, input.experiment.target_commit_sha),
    ...
    fn apply_product_tournament_experience_action(
        runtime: &mut ProductTournamentExperienceRuntime,
        input: &ProductTournamentExperienceInput,
        ...
        match action.id.as_str() {
            "inspect_pain" => {
                ...
                runtime.state = product_tournament_experience_state(
                    ...
                    vec![
                        ...
                        "contender proof still requested".to_owned(),
                    ],
                    false,
                );
            ...
            "abandon_task" => {
                runtime.terminal_status = ProductTournamentExperienceTerminalStatus::Abandoned;
                runtime.state = product_tournament_experience_state(
                    "abandoned",
                    "Task abandoned",
    ...
    fn allowed_product_tournament_experience_actions(
        runtime: &ProductTournamentExperienceRuntime,
    ) -> Vec<ProductTournamentExperienceAllowedAction> {
        if runtime.terminal_status != ProductTournamentExperienceTerminalStatus::InProgress
    ...
    fn current_alternative_label(input: &ProductTournamentExperienceInput) -> String {
        input
            .alternatives
            .first()
    ...
    fn current_switching_objection(input: &ProductTournamentExperienceInput) -> String {
        input
            .alternatives
            .first()
  ...
  private static func appCoreTests() -> String {
    ...
    use app_core::{
        ...
        GuiReplayStep, GuiReplayTrace, ProductTournamentAlternative, ProductTournamentContender,
        ProductTournamentCurrentWorkflow, ProductTournamentDecisionIntent,
        ProductTournamentExperienceAction, ProductTournamentExperienceInput,
        ProductTournamentExperienceTerminalStatus, ProductTournamentExperiment, ProductTournamentPain,
    ...
    fn product_tournament_experience_fixture_replays_allowed_actions_deterministically() {
        let input = ProductTournamentExperienceInput {
            ...
            contender: ProductTournamentContender {
                id: "contender-reporting".to_owned(),
            ...
            experiment: ProductTournamentExperiment {
                id: "experiment-reporting".to_owned(),
                branch_name: "product-tournament/reporting".to_owned(),
                target_commit_sha: "fixture-target-commit".to_owned(),
        ...
        assert_eq!(first.branch_name, "product-tournament/reporting");
        assert_eq!(first.target_commit_sha, "fixture-target-commit");
  ...
  private static func appCLIMain() -> String {
    ...
    use app_core::{
        ...
        DemoState, GuiReplayTrace, ProductTournamentExperienceInput, SimulationInput,
    };
    ...
    fn parse_product_tournament_experience_input(
        mut args: impl Iterator<Item = String>,
    ) -> Result<ProductTournamentExperienceInput, Box<dyn std::error::Error>> {
        let mut input_json = None;
        while let Some(arg) = args.next() {
            match arg.as_str() {
                "--input" => {
        ...
        match input_json {
            Some(value) => Ok(serde_json::from_str(&value)?),
            None => Ok(ProductTournamentExperienceInput::default()),
        }
    }
  ...
  private static func xtaskMain() -> String {
    ...
    fn product_tournament_trace_check() -> Result<()> {
        ...
        let input = r#"{"schemaVersion":2,"pain":{"id":"pain-reporting","summary":"Weekly reporting takes too long","impact":"Managers lose visibility"},"contender":{"id":"contender-compass","title":"Compass workflow helper","promise":"Turn scattered updates into a reviewed weekly report"},"experiment":{"id":"experiment-reporting","branchName":"product-tournament/reporting","targetCommitSha":"demo-target-commit","successSignal":"Persona completes a report draft and sees why it beats the current workflow"},"scenario":{"seed":"demo","personaSummary":"Operations lead evaluating a workflow tool","task":"Reduce weekly reporting work"},"currentWorkflow":{"summary":"Collect updates manually, paste them into a spreadsheet, and chase missing details.","frictionPoints":["manual copy paste","late follow ups"]},"alternatives":[{"id":"spreadsheet","name":"Shared spreadsheet","description":"A manual tracker with copied status updates.","switchingObjection":"The team already knows the spreadsheet."}],"actions":[{"id":"inspect_pain","params":{}},{"id":"compare_current_alternative","params":{}},{"id":"reduce_switching_objection","params":{}},{"id":"start_contender_workflow","params":{}},{"id":"provide_requested_input","params":{}}]}"#;
