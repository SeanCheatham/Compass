...
struct CompassWorkspaceStorageActivationPlan: Equatable {
    ...
    init(
        ...
        let standardizedRepoURL = repoURL.standardizedFileURL
        let identifier = CompassWorkspaceStorageAssessment.projectStorageIdentifier(for: standardizedRepoURL)
    ...
    private static func status(
        ...
        case .candidateMissing:
            return status(
                ...
                detail: "Prepared Application Support candidate is missing at \(boundedPath(candidateURL.path, limit: 128)).",
        ...
        case .candidateIncomplete:
            ...
            return status(
                ...
                recommendation: "Prepare the candidate again before activating Application Support storage.",
                systemImage: "exclamationmark.triangle.fill"
            )
        case .candidateInvalid:
            return status(
                kind: .candidateInvalid,
                label: "Activation blocked",
                detail: "Prepared Application Support candidate is not usable: \(validation.invalidReason ?? "unknown validation failure").",
