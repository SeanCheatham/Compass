...
final class AgentExecutor {
    ...
    private func streamOneTurnWithRetry(
        ...
        for attempt in 1...Self.maxStreamAttempts {
            ...
            do {
                ...
                emit(
                    ...
                    detail: "attempt \(attempt)/\(Self.maxStreamAttempts) in \(String(format: "%.1f", delay))s — \(error.localizedDescription)",
                    kind: .lifecycle,
                    status: .running
                )
        ...
        throw lastError ?? AgentExecutionError.streamFailed("retry loop exhausted with no recorded error")
    }
    ...
    static func shouldRetry(_ error: Error) -> Bool {
        if let openAIError = error as? OpenAIError {
            switch openAIError {
            case let .statusError(_, statusCode):
