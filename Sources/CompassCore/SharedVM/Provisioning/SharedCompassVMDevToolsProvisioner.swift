...
enum SharedCompassVMDevToolsProvisioner {
  ...
  static let installRequestSentinelPath =
  ...
  static let installLaunchDaemonPlistGuestPath =
    "/Library/LaunchDaemons/com.seancheatham.Compass.devtools-install.plist"
  ...
  static func parsePhase(fromLogTail tail: String) -> InstallPhase? {
    ...
    if lower.contains("done.") || lower.contains("install complete")
      || lower.contains("[compass-clt] softwareupdate exit=0")
    {
      return .done
    }
    if lower.contains("installed:") || lower.contains("installed cli")
      || lower.contains("installed command line tools")
    {
      return .installed
    }
    if lower.contains("installing:") || lower.contains("running pre-install")
      || lower.contains("running install") || lower.contains("installing command line tools")
    {
      return .installing
    }
    ...
    if lower.contains("downloading:") || lower.contains("download started")
      || lower.contains("downloading command line tools")
    {
      return .downloading
    }
