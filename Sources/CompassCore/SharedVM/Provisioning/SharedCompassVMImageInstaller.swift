...
extension OSLog {
  static let restoreImageFetch = OSLog(
    subsystem: "com.seancheatham.Compass", category: "RestoreImageFetch")
}
      ...
      let url = try await withCheckedThrowingContinuation { continuation in
        VZMacOSRestoreImage.fetchLatestSupported { result in
          ...
          case .success(let image):
            os_log(
              .info, log: .restoreImageFetch,
              "Catalog fetch succeeded: build %{public}@ URL %{public}@",
...
struct DefaultInstallerRunner: InstallerRunning {
  func runInstaller(
    ...
    guard let requirements = restoreImage.mostFeaturefulSupportedConfiguration else {
      throw NSError(
        ...
        code: 2,
        userInfo: [
          NSLocalizedDescriptionKey: "Restore image has no supported configuration on this host"
        ]
      )
    }
  ...
  private static func runMainActor(
    ...
    installer.progress.addObserver(
      observer, forKeyPath: "fractionCompleted", options: [.new, .initial], context: nil)
    defer {
