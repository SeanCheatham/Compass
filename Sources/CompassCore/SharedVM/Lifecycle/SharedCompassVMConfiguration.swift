...
struct SharedCompassVMConfiguration {
  ...
  static func makeInstallationConfiguration(
    ...
    try platform.hardwareModel.dataRepresentation.write(
      to: inputs.bundle.hardwareModelURL, options: .atomic)
    try platform.machineIdentifier.dataRepresentation.write(
      to: inputs.bundle.machineIdentifierURL, options: .atomic)
  ...
  static func replaceConsoleAttachment(
    ...
    guard let device = configuration.consoleDevices.first as? VZVirtioConsoleDeviceConfiguration
    else {
  ...
  private static func makeGraphicsDevice() -> VZMacGraphicsDeviceConfiguration {
    ...
    device.displays = [
      VZMacGraphicsDisplayConfiguration(
        widthInPixels: 1920, heightInPixels: 1200, pixelsPerInch: 220)
    ]
    return device
  }

  private static func makeStorageDevice(bundle: SharedCompassVMBundle) throws
  ...
  private static func makeNetworkDevice(macAddress: String? = nil)
