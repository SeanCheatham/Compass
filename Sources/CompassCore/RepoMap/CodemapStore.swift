...
package struct CodemapStore: Sendable {
  ...
  package init(directory: URL, prettyPrint: Bool = false) {
  ...
  /// workspace picks.
  package static func defaultDirectory(forWorkspace workspace: CompassWorkspace) -> URL {
  ...
  /// triggers a re-parse.
  package func loadEntry(forRelativePath relativePath: String) -> CodemapEntry? {
  ...
  package func saveEntry(_ entry: CodemapEntry) throws {
  ...
  /// silently OK.
  package func deleteEntry(forRelativePath relativePath: String) throws {
    let url = entryURL(forRelativePath: relativePath)
  ...
  package func loadAllEntries() -> [CodemapEntry] {
  ...
  package func ensureDirectoryExists() throws {
    let fm = FileManager.default
