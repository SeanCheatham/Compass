...
package struct AgentExecutionResult: Equatable {
  package var submitResultArguments: Data
  package var iterations: Int
  package var assistantText: String
...
package struct AgentExecutionConfiguration {
  package var settings: AgentRuntimeSettings
  package var phase: AgentPhase
  package var continuationPhase: AgentContinuationPhase
  package var modelOverride: String
  package var systemPrompt: String
  package var userPrompt: String
  package var tools: [AgentTool]
  package var modelRuntime: (any LocalModelGenerating)?
  package var submitResultSchema: AgentToolParametersSchema
  ...
  package var promptLogLabelPrefix: String?
  ...
  package var validateSubmitResult: (@Sendable (Data) throws -> Void)?
  package var maxIterations: Int
  package var wallClockTimeout: TimeInterval
  ...
  package var contextWindowTokens: Int { settings.contextWindowTokens }
}

package enum AgentExecutionError: LocalizedError, Equatable {
  ...
  /// failure.
  package var isAgentBudgetExhaustion: Bool {
    switch self {
...
package final class AgentExecutor {
  ...
  package static let baseStreamRetryDelay: TimeInterval = 1.0
  package static let maxStreamRetryDelay: TimeInterval = 30.0
  ...
  package static let compactionThresholdFraction: Double = 0.75

  /// Conventional chars-per-token divisor for local token estimates.
  package static let estimatedCharsPerToken: Int = 4
  ...
  package static let maxSummaryCompletionTokens: Int = 8_192

  package let onEvent: @Sendable (LiveEvent) -> Void
  package var cancelled = false

  package init(onEvent: @Sendable @escaping (LiveEvent) -> Void = { _ in }) {
