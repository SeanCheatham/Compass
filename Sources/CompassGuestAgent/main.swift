...
FileHandle.standardError.write(Data("compass-guest-agent: starting, listening on vsock port \(String(format: "0x%08X", port))\n".utf8))
...
do {
    ...
    FileHandle.standardError.write(Data("compass-guest-agent: accept loop terminated: \(error)\n".utf8))
    exit(1)
}
